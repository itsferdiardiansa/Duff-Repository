<template>
  <div class="login-page">
    ...
    <div class="content">
      <div class="bg-img">
        <img svg-inline src="@icon/login.svg" />
      </div>
...
import Form from '@page/UIKit/components/Form';
import FormControl from '@page/UIKit/components/FormControl';
  ...
  setup() {
    ...
    const onSubmit = () => {
      ...
      form.validate(async (valid, fields) => {
        if (valid)
          try {
            const checkLogin = await store.dispatch(
              'user/doLogin',
              unref(fields)
            );
