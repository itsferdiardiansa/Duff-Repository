...
export default {
  ...
  setup(props, { slots }) {
    ...
    emits.on('show-alert', e => {
      ...
      if (Reflect.has(e, 'variant')) variant.value = e.variant;

      if (Reflect.has(e, 'timeout')) timeout.value = e.timeout;
    });
