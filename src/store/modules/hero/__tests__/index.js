...
import collections from '__mock__/services/hero/collections';
...
describe('module/Hero', () => {
  ...
  it('it will mutate the error state when it fails fetch data', async () => {
    ...
    mockGetListHero.mockClear();
    mockGetListHero.mockRejectedValueOnce(errorResponse);
  ...
  it('will mutate the state items and have property properly', async () => {
    ...
    items.forEach(item => {
      expect(item).toMatchObject({
        ...
        banner_mobile: expect.any(String),
        banner_type: expect.any(String),
        button_text: expect.any(String),
        button_url: expect.any(String),
        description: expect.any(String),
        description_color: expect.any(String),
        is_active: expect.any(String),
        position: expect.any(String),
        promoter: expect.any(String),
        text_color: expect.any(String),
        title: expect.any(String),
        title_color: expect.any(String),
  ...
  it('will mutate the state error when failed post data', async () => {
    ...
    mockCreateHero.mockRejectedValueOnce(errorResponse);
  ...
  it('will mutate the state error when failed update data', async () => {
    ...
    mockUpdateHero.mockReset();
    mockUpdateHero.mockRejectedValueOnce(errorResponse);
  ...
  it('will mutate the state error when failed delete data', async () => {
    ...
    mockDeleteHero.mockReset();
    mockDeleteHero.mockRejectedValueOnce(errorResponse);
