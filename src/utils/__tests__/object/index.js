...
describe('utils/collectObjectKeys', () => {
  it('should return the same value as the object or array parameter', () => {
    let filter

    for (let item of [0, 1]) {
      filter = item
        ? collectObjectKeys(data.dummy)
        : collectObjectKeys([].concat(data.dummy))
